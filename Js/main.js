...
z.open("GET","https://api.weatherapi.com/v1/search.json?key=aff765a12f074d15a44214155233112&q=lond")
z.send()
...
//   document.getElementById("month").innerHTML=monthName
  ...
  async function search(a) {
    var xml = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=aff765a12f074d15a44214155233112&q=${a}&days=3`);
    if (xml.ok && 400 != xml.status) {
        var a = await xml.json();
        display(a.location, a.current),
        displayForecast(a.forecast.forecastday)
    }
}
                    ...
                    <div class="degree">     
                       <div class="num">${xml.temp_c}<sup>o</sup>C</div>  
                        <div class="forecast-icon">
                                 <img src="https:${xml.condition.icon}" alt="" width=90>    
                      </div>  
                       </div>
